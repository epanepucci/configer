...
# Redis service class with ReJSON operations
class RedisService:
    def __init__(self, redis_client):
        self.redis = redis_client
        # For RedisJSON operations
        self.json = Client(host=settings.REDIS_HOST, port=settings.REDIS_PORT, 
    ...
    async def add_instrument(self, instrument_id, metadata):
        ...
        if not await self.redis.exists("instruments:list"):
            await self.redis.json().set("instruments:list", Path.root_path(), {})
        
        # Add instrument to list
        await self.redis.json().set(f"instruments:list", f".{instrument_id}", metadata)
        
        # Initialize empty config
        await self.redis.json().set(f"instrument:{instrument_id}:config", Path.root_path(), {})
        
        # Initialize empty versions and snapshots lists
        await self.redis.json().set(f"instrument:{instrument_id}:versions", Path.root_path(), [])
        await self.redis.json().set(f"instrument:{instrument_id}:snapshots", Path.root_path(), [])
    ...
    async def update_config(self, instrument_id, config_data, user, comment=""):
        ...
        await self.redis.json().set(f"instrument:{instrument_id}:config", Path.root_path(), config_data)
        ...
        await self.redis.json().arrappend(
            f"instrument:{instrument_id}:versions",
            Path.root_path(),
            version_id
        )
    ...
    async def get_version(self, instrument_id, version_id):
        ...
        version = await self.redis.json().get(f"instrument:{instrument_id}:version:{version_id}")
        return version
    ...
    async def create_snapshot(self, instrument_id, snapshot_name, description, user):
        ...
        await self.redis.json().arrappend(
            f"instrument:{instrument_id}:snapshots",
            Path.root_path(),
            snapshot_name
        )
    ...
    async def get_snapshot(self, instrument_id, snapshot_name):
        ...
        snapshot = await self.redis.json().get(f"instrument:{instrument_id}:snapshot:{snapshot_name}")
        return snapshot
